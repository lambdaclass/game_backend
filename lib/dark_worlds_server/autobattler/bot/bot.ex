defmodule DarkWorldsServer.AutoBattler.Bot do
  ...
  @visibility_max_range_cells 100000
  ...
  #######################
  @impl GenServer
  def init(%{runner_pid: runner_pid, config: config, broadcast_topic: broadcast_topic} = state) do
    Phoenix.PubSub.subscribe(DarkWorldsServer.PubSub, broadcast_topic)
  ...
  def handle_info({:do_action, bot_id}, state) do
    ...
    if bot_state.alive do
      ...
      do_action(state, bot_state, bot_id)
    end
    ...
    bots =
      Enum.reduce(players, state.bots, fn player, acc_bots ->
        case {player.health <= 0, acc_bots[player.id]} do
          {true, bot} when not is_nil(bot) ->
            IO.inspect("player #{player.id} died!")
            put_in(acc_bots, [player.id, :alive], false)
          _ -> acc_bots
        end
      end)
  ...
  defp do_action(%{runner_pid: runner_pid}, %{action: {:move, angle}}, player_id) do
    IO.inspect("Player #{player_id} moving")
    Runner.move(runner_pid, player_id, %Move{angle: angle}, timestamp())
  end

  defp do_action(%{runner_pid: runner_pid}, %{action: {:attack, %{type: :enemy, attacking_angle_direction: angle}, skill}}, player_id) do
    IO.inspect("Player #{player_id} attacking")
    Runner.attack(runner_pid, player_id, %UseSkill{angle: angle, skill: skill}, timestamp())
  end

  defp do_action(_state, _bot_state, _player_id) do
    nil
  end

  defp timestamp(), do: DateTime.utc_now() |> DateTime.to_unix(:millisecond)
  ...
  def decide_objective(bot_state, %{game_state: game_state, config: config}, bot_id, %{enemies_by_distance: enemies_by_distance}) do
    bot = Enum.find(game_state.players, fn player -> player.id == bot_id end)

    closest_entity = List.first(enemies_by_distance)
  ...
  defp set_objective(bot_state, _bot, _game_state, _config, _closest_entity) do
