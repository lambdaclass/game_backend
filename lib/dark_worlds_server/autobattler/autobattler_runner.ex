defmodule DarkWorldsServer.AutoBattler.RunnerSupervisor.Runner do
  use GenServer, restart: :transient
  require Logger
  alias DarkWorldsServer.AutoBattler.Bot.BotSupervisor
  ...
  def get_game_state(runner_pid) do
    GenServer.call(runner_pid, :get_game_state)
  end
  ...
  def join(runner_pid, user_id, character_name) do
    GenServer.cast(runner_pid, {:join, user_id, character_name})
  end
  ...
  def init(bot_config) do
    ...
    Process.put(:map_size, {backend_config.game.width, backend_config.game.height})

    NewRelic.increment_custom_metric("GameBackend/TotalGames", 1)

    bot_pids = insert_players(bot_config, state.broadcast_topic)

    state = Map.put(state, :bot_pids, bot_pids)

    {:ok, state}
  end

  defp insert_players(bot_config, broadcast_topic) do
    # Enum.reduce(team_config, & BotSupervisor.start_bot(self(), &1))
    BotSupervisor.start_bots(self(), bot_config, broadcast_topic)
  end
  ...
  @impl true
  def handle_cast({:join, user_id, character_name}, state) do
